...
class ContextSession {
  ...
  initFromCookie() {
    ...
    try {
      ...
      if (!(err instanceof SyntaxError)) {
        ...
        if (opts.domain && Array.isArray(opts.domain)) {
          for (const domain of opts.domain) {
            const optsOverride = Object.assign({}, opts, { domain });
            ctx.cookies.set(opts.key, '', optsOverride);
          }
        } else {
  ...
  * remove() {
    ...
    if (opts.domain && Array.isArray(opts.domain)) {
      for (const domain of opts.domain) {
        const optsOverride = Object.assign({}, opts, { domain });
        ctx.cookies.set(key, '', optsOverride);
      }
    } else {
  ...
  * save(changed) {
    ...
    if (externalKey) {
      ...
      if (opts.domain && Array.isArray(opts.domain)) {
        let matchingDomain = this.getDomain(this.ctx);

        if (opts.domain.includes(matchingDomain)) {
          let domain = matchingDomain == 'localhost' ? matchingDomain : `.${matchingDomain}`;
          const optsOverride = Object.assign({}, opts, { domain });

          this.ctx.cookies.set(key, externalKey, optsOverride);
        } else {
          const optsOverride = Object.assign({}, opts, { domain: opts.domains[0] });
          this.ctx.cookies.set(key, externalKey, optsOverride);
        }
      } else {
    ...
    if (opts.domain && Array.isArray(opts.domain)) {
      ...
      if (opts.domain.includes(matchingDomain)) {
        ...
        this.ctx.cookies.set(key, json, optsOverride);
      } else {
        console.log('koa-session: no matchin domain for: ', matchingDomain);
        const optsOverride = Object.assign({}, opts, { domain: opts.domain[0] });
  ...
  getDomain(ctx) {
    ...
    if (segments.length > 2) {
      matchingDomain = segments.splice(-2).join('.');
    } else {
      matchingDomain = host
    }
